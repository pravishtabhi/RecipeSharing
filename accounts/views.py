...
from .serializers import UserSerializer, UserProfileCurrentUserSerializer
...
class registrationView(APIView):
    ...
    def post(self, request):
        if not request.data.get('email'):
            return Response({"error": {"email": ["This field is required."]}}, status=status.HTTP_400_BAD_REQUEST)
...
class UserProfile(APIView):
    ...
    def get(self,request):
        user = request.user
        user_serilizer = UserProfileCurrentUserSerializer(user)
        if user_serilizer:
            return Response({"data": user_serilizer.data}, status=status.HTTP_200_OK)
        return Response({"error": user_serilizer.errors}, status=status.HTTP_400_BAD_REQUEST)
